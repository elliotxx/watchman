...
class EditTemplate extends React.Component {
    constructor(props) {
        ...
        if (props.location.state && props.location.state.template) {
            ...
            this.state.template = props.location.state.template;
        }
    }
        ...
        this.props.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                ...
                if (this.state.isEdit)
                    values.ID = this.state.template.ID;
                method(globalConfig.rootPath + '/api/v1/template', values)
                    .then(res => {
                        console.log(res);
                        if (res.status === 200) {
                            if (this.state.isEdit)
        ...
        return (
            <div id="template-form">
                <Form onSubmit={this.handleSubmit}>
                    {/*<PageHeader onBack={() => this.props.history.goBack()} title="任务模板配置" subTitle="This is a subtitle" />*/}
                    <Typography.Title level={4} type="secondary"><Icon type="left" onClick={() => this.props.history.goBack()} style={{marginRight: '5px'}}/> 任务模板配置</Typography.Title>
                    <hr/>
                    <Form.Item label="模板名称">
                        {getFieldDecorator('name', {
                            ...
                            initialValue: this.state.isEdit? this.state.template.name : '',
                        })(<Input />)}
                    </Form.Item>
                    ...
                    <Form.Item label="定时配置">
                        {getFieldDecorator('cron', {
                            ...
                            initialValue: this.state.isEdit? this.state.template.cron : '',
                    ...
                    >
                        {getFieldDecorator('pattern', {
                            ...
                            initialValue: this.state.isEdit? this.state.template.pattern : '',
                        })(<TextArea rows={4} />)}
                    </Form.Item>

                    <Form.Item label="邮件内容" extra="内容中可使用变量: %name% 代表任务模板名称，%target% 代表抓取规则匹配到的目标" >
                        {getFieldDecorator('content', {
                            ...
                            initialValue: this.state.isEdit? this.state.template.content : '',
                        })(<TextArea rows={4} />)}
                    </Form.Item>
